...
* **fis install <name>:** 命令安装fis仓库提供的各种 **组件、框架、示例、素材、配置等** 开发资源。
...
在FIS中，如果CSS引用图片时，添加了 `?__sprite` query，那么CSS在使用 `pack` 设置进行合并时，不仅仅会合并CSS文件，还会将所有标识了合并的图片进行合并处理。
...
如果对FIS的运行原理和设计思想感兴趣，可以参考[更多文档](/docs/dev/more.html)。
