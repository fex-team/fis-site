...
<!-- 让我们快速上手，一起试试看如何使用FIS轻松的完成一个传统前端项目的[资源压缩](http://fex.baidu.com/fis-site/docs/beginning/getting-started.html#资源压缩)、[添加md5戳](http://fex.baidu.com/fis-site/docs/beginning/getting-started.html#添加md5戳)、[资源合并](http://fex.baidu.com/fis-site/docs/beginning/getting-started.html#资源合并)等性能优化工作。 -->
...
如果觉得参数输入比较麻烦，实际上也支持参数缩写，更多的参数可以参考 ```fis release -h``` 或[命令行](http://fex.baidu.com/fis-site/docs/api/cli.html)。
...
<!-- > 细心的朋友可能还会发现，index.html中原本使用相对路径对资源定位，在我们的构建产出中已经全部修改为了**绝对路径**，这是因为FIS构建工具内置了[三种语言能力](http://fex.baidu.com/fis-site/docs/more/fis-standard.html)，其中资源定位功能会将所有路径引用调整为绝对路径。
...
FIS可以通过[pack](http://fex.baidu.com/fis-site/docs/api/fis-conf.html#pack)来进行资源文件的合并，比如我们需要将DEMO中的公共库文件打包在一起，可以修改 fis-conf.js 配置，加入pack配置
...
> FIS的编译系统拥有一个使用简单、扩展方便的插件体系，这套插件体系保证了FIS编译工具的灵活性和生命力。这里我们简单介绍一下FIS插件的安装方法，更加详细的插件系统介绍可以查看配置API [modules](http://fex.baidu.com/fis-site/docs/api/fis-conf.html#modules)。
...
* 阅读[辅助开发](http://fex.baidu.com/fis-site/docs/beginning/assist.html)了解除了构建能力外，FIS提供的**文件监听、自动编译、自动刷新**等开发辅助能力。
* 阅读[命令行](http://fex.baidu.com/fis-site/docs/api/cli.html)全面了解FIS命令行参数。
* 阅读[语言能力](http://fex.baidu.com/fis-site/docs/more/fis-standard.html)了解FIS的语言扩展能力。
* 阅读[配置API](http://fex.baidu.com/fis-site/docs/api/fis-conf.html)了解FIS完整的功能配置，由于内容较多，可以先大概浏览内容，使用时再详细查阅。
* 有兴趣的同学还可以阅读[更多文档](http://fex.baidu.com/fis-site/docs/dev/more.html)了解FIS的基础原理与设计思想，让你可以更好的理解和使用FIS打造自己解决方案。
...
实际上这些问题都可以用前端模块化来解决，通过前端模块化开发，我们可以不再担心各种资源加载问题，就像编写Node.js程序一样编写前端项目。那么除了业界流行的各种前端模块化加载库，FIS也提供了一种新的思路来解决模块化加载问题，点击[前端模块化](http://fex.baidu.com/fis-site/docs/advance/modjs-solution.html)了解更多。

如果对FIS的运行原理和设计思想感兴趣，可以参考[更多文档](http://fex.baidu.com/fis-site/docs/dev/more.html)。
