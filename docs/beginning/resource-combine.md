...
本篇指南使用的示例与资源压缩指南中使用的是相同的示例，示例项目的准备工作可以参考[资源压缩](http://fex.baidu.com/fis-site/docs/beginning/getting-started.html)
...
如果觉得参数输入比较麻烦，实际上也有等价的更短的命令可以灵活组合，更多的参数可以参考[命令行](http://fex.baidu.com/fis-site/docs/api/cli.html)。
...
我们可以通过[pack](http://fex.baidu.com/fis-site/docs/api/fis-conf.html#pack)设置来干预合并结果。人工干预的必要性在于我们可以将类似underscore、jquery、backbone等基础库固定打包，首先我们可以让不同页面之间公用这些基础库而不用重新下载，其次由于基础库不容易改变，这种策略也对缓存更加友好。
