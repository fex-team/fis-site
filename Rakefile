...
def check_destination
  unless Dir.exist? CONFIG["git_folder"]
    sh "git clone https://#{ENV['GIT_NAME']}:#{ENV['GH_TOKEN']}@github.com/#{USERNAME}/#{REPO}.git #{CONFIG["git_folder"]}"
  end
end
...
namespace :site do
  ...
  task :deploy do
    ...
    Dir.chdir(CONFIG["git_folder"]) { sh "git checkout #{DESTINATION_BRANCH}" }

    # Generate the site
    sh "bundle exec jekyll build"

    # Build with fis
    Dir.chdir(CONFIG["fis_source"]) { sh "fis release -ompDd #{CONFIG["fis_dest"]}" }

    # Commit and push to github
    sha = `git log`.match(/[a-z0-9]{40}/)[0]
    Dir.chdir(CONFIG["git_folder"]) do
      sh "git add --all ."
