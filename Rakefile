...
def check_destination
  unless Dir.exist? CONFIG["git_folder"]
    sh "git clone https://#{USERNAME}:#{ENV['GH_TOKEN']}@github.com/#{USERNAME}/#{REPO}.git #{CONFIG["git_folder"]}"
  end
end
...
namespace :site do
  ...
  task :deploy do
    ...
    # Build with fis
    Dir.chdir(CONFIG["destination"]) { sh "fis release -ompDd #{CONFIG["fis_dest"]}" }
